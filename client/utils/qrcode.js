...
export function onQrCodeScan(imageBuffer, cartStore) {
  return new Promise((resolve /*, reject*/) => {
    ...
    //create a worker to handle the big processing for us
    let worker = new Worker('qr-worker.js');
    //send a message to the worker script
    worker.postMessage(imageBuffer);
    worker.onmessage = event => {
      //console.log('event', event);
      //the event message comes back with
      //a property called data that we need
      //to then grab our data property
      //to resolve the promise with
